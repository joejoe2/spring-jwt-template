...
class PasswordServiceTest {
  ...
  void changePasswordOfWithIllegalArgument() {
    ...
    assertThrows(
        ...
        () -> passwordService.changePasswordOf("invalid_uid", "pa55ward", "pa55ward123"));
    assertThrows(
        IllegalArgumentException.class,
        () ->
            passwordService.changePasswordOf(
                user.getId().toString(), "pa55ward", "invalid_password"));
  }
  ...
  void changePasswordOfWithInvalidOperation() {
    ...
    assertThrows(
        ...
        () -> passwordService.changePasswordOf(user.getId().toString(), "incorrect", "pa55ward"));
    // test if old password==new password
    assertThrows(
        InvalidOperation.class,
        () -> passwordService.changePasswordOf(user.getId().toString(), "pa55ward", "pa55ward"));
  }
  ...
  void changePasswordOfWithDoesNotExist() {
    ...
    assertThrows(
        ...
        () -> passwordService.changePasswordOf(finalId.toString(), "pa55ward", "pa55ward"));
  }
        ...
        () -> {
          ...
          assertTrue(
              passwordEncoder.matches(
                  "pa55ward123", userRepository.findById(user.getId()).get().getPassword()));
        });
  }
  ...
  void requestResetPasswordWithIllegalArgument() {
    ...
    assertThrows(
        IllegalArgumentException.class,
        () -> passwordService.requestResetPasswordToken("invalid email"));
  }
  ...
  void requestResetPasswordWithDoesNotExist() {
    ...
    assertThrows(
        UserDoesNotExist.class, () -> passwordService.requestResetPasswordToken("not@email.com"));
  }
  ...
  void resetPasswordWithInvalidOperation() {
    ...
    assertThrows(
        InvalidOperation.class, () -> passwordService.resetPassword("invalid token", "pa55ward"));
    ...
    assertThrows(
        InvalidOperation.class,
        () -> passwordService.resetPassword(token.getToken(), "newPa55ward"));
  }
  ...
  void resetPassword() {
    ...
    assertDoesNotThrow(
        () -> {
          ...
          assertTrue(
              passwordEncoder.matches(
                  "a12345678", userRepository.findById(user.getId()).get().getPassword()));
        });
  }
}
