...
class RoleServiceTest {
  ...
  void changeRoleOfWithIllegalArgument() {
    ...
    assertThrows(
        IllegalArgumentException.class, () -> roleService.changeRoleOf("invalid_uid", Role.ADMIN));
    // test with not exist user
    assertThrows(
        UserDoesNotExist.class,
        () -> roleService.changeRoleOf(UUID.randomUUID().toString(), Role.STAFF));
    // test the only(default) admin
    UUID id = userRepository.getByRole(Role.ADMIN).get(0).getId();
    assertThrows(
        InvalidOperation.class, () -> roleService.changeRoleOf(id.toString(), Role.NORMAL));
  }
  ...
  void changeRoleOfWithInvalidOperation() {
    ...
    assertThrows(
        ...
        () -> roleService.changeRoleOf(user.getId().toString(), Role.NORMAL));
    // test if role does not change
    assertThrows(
        InvalidOperation.class,
        () -> roleService.changeRoleOf(user.getId().toString(), Role.ADMIN));
