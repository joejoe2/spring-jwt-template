...
class VerificationServiceTest {
  ...
  void issueVerificationCodeWithIllegalArgument() {
    ...
    assertThrows(
        ...
        () -> verificationService.issueVerificationCode("not a email"));
  }
  ...
  void issueVerificationCode() {
    ...
    assertEquals(
        verificationPair.getCode(),
        verificationCodeRepository
            .findById(UUID.fromString(verificationPair.getKey()))
            .get()
            .getCode());
  }
  ...
  void verifyWithIllegalArgument() {
    ...
    assertThrows(
        IllegalArgumentException.class,
        () -> verificationService.verify("invalid key", "test@email.com", "1234"));
    assertThrows(
        IllegalArgumentException.class,
        () -> verificationService.verify(UUID.randomUUID().toString(), "not a email", "1234"));
    assertThrows(
        IllegalArgumentException.class,
        () -> verificationService.verify(UUID.randomUUID().toString(), "test@email.com", null));
  }
  ...
  void verifyWithInvalidOperation() {
    ...
    assertThrows(
        ...
        () -> verificationService.verify(UUID.randomUUID().toString(), "test@email.com", "1234"));
  }
    ...
    assertDoesNotThrow(
        () ->
            verificationService.verify(
                verificationCode.getId().toString(),
